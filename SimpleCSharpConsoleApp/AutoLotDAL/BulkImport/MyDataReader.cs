...
namespace AutoLotDAL.BulkImport
{
    public class MyDataReader<T> : IMyDataReader<T>
    {
        ...
        public int GetValues(object[] values)
        {
            ...
            values = new object[_propertyInfos.Length];
            for (int i = 0; i < _propertyInfos.Length; i++)
            {
                Console.WriteLine(_propertyInfos[i].GetValue(Records[_currentIndex]).ToString());
                values[i] = _propertyInfos[i].GetValue(Records[_currentIndex]);
            }
